...
export const phase04Questions: QuizQuestion[] = [
  ...
  // === Drift detection extra questions (p4-q41 to p4-q48) ===
  {
    id: 'p4-q41',
    phaseId: 'phase-04',
    lessonId: 'p4-drift',
    question: 'Quelle est la différence fondamentale entre le data drift et le concept drift ?',
    options: [
      'Le data drift concerne les données structurées, le concept drift les données non structurées',
      'Le data drift est un changement dans la distribution des entrées P(X), le concept drift est un changement dans la relation entrées-sortie P(Y|X)',
      'Le data drift est détectable, le concept drift est impossible à détecter',
      'Il n\'y a pas de différence, les termes sont interchangeables',
    ],
    correctIndex: 1,
    explanation: 'Le data drift (covariate shift) survient quand P(X) change (ex: nouvelles données démographiques) mais P(Y|X) reste stable. Le concept drift est plus dangereux : la relation P(Y|X) elle-même change (ex: pendant le COVID, les habitudes d\'achat ont radicalement changé). Le concept drift invalide directement le modèle.',
    difficulty: 'easy',
  },
  {
    id: 'p4-q42',
    phaseId: 'phase-04',
    lessonId: 'p4-drift',
    question: 'Qu\'est-ce que le prior probability shift (label drift) ?',
    options: [
      'Un changement dans les hyperparamètres du modèle',
      'Un changement dans la distribution des classes au fil du temps',
      'Un bug dans le pipeline de données',
      'Une modification de l\'architecture du modèle',
    ],
    correctIndex: 1,
    explanation: 'Le label drift (prior probability shift) survient quand la distribution des classes change. Par exemple, le ratio fraude/non-fraude passe de 1% à 5%, ou la proportion de spam dans les emails augmente. Cela peut dégrader les performances du modèle, surtout si le seuil de décision n\'est pas adapté.',
    difficulty: 'easy',
  },
  {
    id: 'p4-q43',
    phaseId: 'phase-04',
    lessonId: 'p4-drift',
    question: 'Quels sont les quatre sous-types de concept drift selon leur dynamique temporelle ?',
    options: [
      'Linéaire, exponentiel, logarithmique, constant',
      'Soudain, graduel, récurrent, incrémental',
      'Rapide, lent, périodique, aléatoire',
      'Local, global, régional, universel',
    ],
    correctIndex: 1,
    explanation: 'Le concept drift soudain est un changement brusque (nouvelle réglementation). Le graduel est une évolution lente des comportements. Le récurrent suit des patterns saisonniers ou cycliques. L\'incrémental est composé de micro-changements continus qui s\'accumulent. Chaque type nécessite une stratégie de détection différente.',
    difficulty: 'medium',
  },
  {
    id: 'p4-q44',
    phaseId: 'phase-04',
    lessonId: 'p4-drift',
    question: 'À partir de quelle valeur de PSI (Population Stability Index) considère-t-on généralement qu\'un drift significatif s\'est produit ?',
    options: [
      'PSI > 0.01',
      'PSI > 0.2',
      'PSI > 1.0',
    ...
    correctIndex: 1,
    explanation: 'Les seuils PSI standards sont : PSI < 0.1 (pas de drift significatif), 0.1 ≤ PSI < 0.2 (drift modéré, surveillance recommandée), PSI ≥ 0.2 (drift significatif nécessitant une action comme le réentraînement). Le PSI compare deux distributions en calculant la somme des différences pondérées par le log des ratios.',
    difficulty: 'medium',
  },
  {
    id: 'p4-q45',
    phaseId: 'phase-04',
    lessonId: 'p4-drift',
    question: 'Comment l\'algorithme ADWIN détecte-t-il le concept drift ?',
    options: [
      'En comparant les performances avec un modèle de référence fixe',
      'En maintenant une fenêtre de taille variable et en détectant les changements statistiques significatifs entre les sous-fenêtres',
      'En réentraînant le modèle à intervalles réguliers et en comparant les résultats',
      'En analysant le code source pour trouver des bugs',
    ],
    correctIndex: 1,
    explanation: 'ADWIN (ADaptive WINdowing) maintient une fenêtre de données qui grandit tant que les données sont stationnaires. Il coupe la fenêtre en deux et teste si les moyennes des sous-fenêtres diffèrent significativement. Si oui, les anciennes données sont supprimées. Cela permet une détection adaptive sans paramètre de taille de fenêtre fixe.',
    difficulty: 'hard',
  },
  {
    id: 'p4-q46',
    phaseId: 'phase-04',
    lessonId: 'p4-drift',
    question: 'Quelle stratégie de réentraînement est recommandée lorsque du concept drift récurrent (saisonnier) est détecté ?',
    options: [
      'Réentraîner une seule fois avec toutes les données historiques',
      'Utiliser des modèles ensemblistes avec des fenêtres temporelles ou des modèles spécialisés par saison, activés dynamiquement',
      'Supprimer toutes les données anciennes et réentraîner uniquement sur les données récentes',
      'Ignorer le drift saisonnier car il se corrige automatiquement',
    ],
    correctIndex: 1,
    explanation: 'Pour le drift récurrent, les approches efficaces incluent : les ensembles de modèles spécialisés par période (un modèle été vs hiver, activé selon la saison), les fenêtres temporelles pondérées (données récentes plus importantes), et les meta-learners qui détectent le régime actuel. Supprimer les anciennes données perdrait la connaissance des patterns cycliques.',
    difficulty: 'hard',
  },
  {
    id: 'p4-q47',
    phaseId: 'phase-04',
    lessonId: 'p4-drift',
    question: 'Quelles métriques de monitoring des features sont les plus utiles pour détecter le data drift en production ?',
    options: [
      'Uniquement la moyenne et l\'écart-type',
      'Statistiques descriptives (moyenne, écart-type, quantiles), tests statistiques (KS, PSI), taux de valeurs manquantes et détection d\'outliers',
      'Le temps de réponse de l\'API et l\'utilisation CPU',
      'Le nombre de requêtes par seconde',
    ],
    correctIndex: 1,
    explanation: 'Un monitoring complet des features combine : les statistiques descriptives pour détecter les changements de distribution, les tests statistiques formels (KS-test pour les features continues, chi-squared pour les catégorielles, PSI pour la stabilité globale), le suivi des valeurs manquantes et des outliers qui peuvent signaler des problèmes de pipeline.',
    difficulty: 'medium',
  },
  {
    id: 'p4-q48',
    phaseId: 'phase-04',
    lessonId: 'p4-drift',
    question: 'Quel outil open-source est spécifiquement conçu pour le monitoring de drift des modèles ML en production ?',
    options: [
      'Grafana',
      'Evidently AI',
      'Jenkins',
    ...
    correctIndex: 1,
    explanation: 'Evidently AI est un outil open-source dédié au monitoring ML qui génère des rapports de drift automatisés : data drift (par feature), prediction drift, target drift et data quality. Il s\'intègre dans les pipelines existants et peut produire des dashboards HTML ou des métriques pour Prometheus/Grafana. NannyML est une alternative populaire.',
    difficulty: 'medium',
  },
];
