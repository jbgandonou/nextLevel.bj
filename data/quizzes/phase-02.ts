...
export const phase02Questions: QuizQuestion[] = [
  ...
  // === MLflow extra questions (p2-q41 to p2-q48) ===
  {
    id: 'p2-q41',
    phaseId: 'phase-02',
    lessonId: 'p2-mlflow',
    question: 'Quels sont les quatre composants principaux de MLflow ?',
    options: [
      'Training, Testing, Deployment, Monitoring',
      'Tracking, Projects, Models, Model Registry',
      'Data, Features, Models, Endpoints',
      'Notebooks, Pipelines, Artifacts, Dashboard',
    ],
    correctIndex: 1,
    explanation: 'MLflow s\'articule autour de quatre composants : Tracking (enregistrement des paramètres, métriques et artefacts), Projects (format de packaging reproductible), Models (format standard multi-framework), et Model Registry (gestion du cycle de vie des modèles avec staging/production).',
    difficulty: 'easy',
  },
  {
    id: 'p2-q42',
    phaseId: 'phase-02',
    lessonId: 'p2-mlflow',
    question: 'Dans MLflow Tracking, quelle est la différence entre un paramètre (param) et une métrique (metric) ?',
    options: [
      'Les paramètres sont numériques, les métriques sont textuelles',
      'Les paramètres sont les entrées de l\'expérience (hyperparamètres), les métriques sont les résultats mesurés (performance)',
      'Les paramètres sont obligatoires, les métriques sont optionnelles',
      'Il n\'y a pas de différence fonctionnelle',
    ],
    correctIndex: 1,
    explanation: 'Les paramètres (log_param) enregistrent les entrées de l\'expérience : hyperparamètres comme le learning rate, batch size, nombre d\'epochs. Les métriques (log_metric) enregistrent les résultats : accuracy, loss, F1-score. Les métriques peuvent être loggées à chaque step pour suivre l\'évolution de l\'entraînement.',
    difficulty: 'easy',
  },
  {
    id: 'p2-q43',
    phaseId: 'phase-02',
    lessonId: 'p2-mlflow',
    question: 'Quel est le rôle du Model Registry dans MLflow ?',
    options: [
      'Entraîner automatiquement les modèles',
      'Gérer le cycle de vie des modèles avec des étapes (Staging, Production, Archived) et le versioning',
      'Scanner les modèles pour détecter les vulnérabilités',
      'Convertir les modèles entre différents frameworks',
    ],
    correctIndex: 1,
    explanation: 'Le Model Registry centralise la gestion des modèles : versioning (v1, v2...), transitions d\'étapes (None → Staging → Production → Archived), annotations et descriptions, et lignage (lien vers le run d\'entraînement). Il facilite la gouvernance et le déploiement contrôlé des modèles.',
    difficulty: 'medium',
  },
  {
    id: 'p2-q44',
    phaseId: 'phase-02',
    lessonId: 'p2-mlflow',
    question: 'Quel risque de sécurité est associé à un serveur MLflow exposé sans authentification ?',
    options: [
      'Les expériences s\'exécutent plus lentement',
      'Un attaquant peut accéder aux modèles, aux données d\'entraînement et potentiellement injecter des modèles malveillants',
      'Les métriques de performance sont faussées',
      'Le serveur consomme trop de RAM',
    ],
    correctIndex: 1,
    explanation: 'Un serveur MLflow sans authentification expose les artefacts (modèles sérialisés, datasets), les paramètres (qui peuvent contenir des informations sensibles) et permet à un attaquant d\'enregistrer des modèles malveillants dans le registry. Il est crucial de sécuriser MLflow avec une authentification, un reverse proxy et des contrôles d\'accès réseau.',
    difficulty: 'medium',
  },
  {
    id: 'p2-q45',
    phaseId: 'phase-02',
    lessonId: 'p2-mlflow',
    question: 'Quelle commande permet de lancer l\'interface web de MLflow avec un backend de stockage persistant ?',
    options: [
      'mlflow run --port 5000',
      'mlflow server --backend-store-uri sqlite:///mlflow.db --host 0.0.0.0',
      'mlflow start --database sqlite',
      'mlflow ui --persistent --db mlflow.sqlite',
    ],
    correctIndex: 1,
    explanation: 'La commande "mlflow server" lance le serveur de tracking avec un backend persistant. Le flag --backend-store-uri spécifie la base de données (SQLite, PostgreSQL, MySQL). Le flag --default-artifact-root définit où stocker les artefacts (local, S3, GCS). Sans backend persistant, les données sont perdues au redémarrage.',
    difficulty: 'medium',
  },
  {
    id: 'p2-q46',
    phaseId: 'phase-02',
    lessonId: 'p2-mlflow',
    question: 'Comment MLflow assure-t-il la reproductibilité des expériences ML ?',
    options: [
      'En stockant uniquement les résultats finaux',
      'En enregistrant l\'environnement complet (dépendances, code, paramètres, données) via MLflow Projects et le tracking automatique',
      'En forçant l\'utilisation d\'un seul framework ML',
      'En exécutant toutes les expériences sur le même serveur',
    ],
    correctIndex: 1,
    explanation: 'MLflow Projects définit l\'environnement (conda.yaml ou Dockerfile), le code source (git commit) et les paramètres d\'entrée. Combiné au Tracking qui enregistre chaque paramètre, métrique et artefact, toute expérience peut être exactement reproduite. L\'autologging capture automatiquement les détails de chaque framework.',
    difficulty: 'hard',
  },
  {
    id: 'p2-q47',
    phaseId: 'phase-02',
    lessonId: 'p2-mlflow',
    question: 'Quelle bonne pratique de sécurité est recommandée pour le stockage des artefacts MLflow en production ?',
    options: [
      'Stocker les artefacts sur le disque local du serveur MLflow',
      'Utiliser un stockage objet chiffré (S3 avec SSE-KMS, GCS) avec des politiques IAM restrictives',
      'Partager un dossier réseau NFS sans authentification pour la performance',
      'Stocker les artefacts dans la base de données SQLite',
    ],
    correctIndex: 1,
    explanation: 'En production, les artefacts (modèles, datasets) doivent être stockés sur un stockage objet chiffré (S3, GCS, Azure Blob) avec des politiques IAM granulaires. Le chiffrement au repos (SSE-KMS) protège les modèles comme propriété intellectuelle. Le versioning du bucket protège contre les suppressions accidentelles.',
    difficulty: 'hard',
  },
  {
    id: 'p2-q48',
    phaseId: 'phase-02',
    lessonId: 'p2-mlflow',
    question: 'Qu\'est-ce que l\'autologging dans MLflow et quel est son avantage ?',
    options: [
      'La génération automatique de logs système pour le débogage',
      'L\'enregistrement automatique des paramètres, métriques et modèles sans code explicite, via mlflow.autolog()',
      'La rotation automatique des fichiers de logs',
      'L\'envoi automatique des logs vers un SIEM',
    ],
    correctIndex: 1,
    explanation: 'mlflow.autolog() détecte automatiquement le framework utilisé (scikit-learn, TensorFlow, PyTorch, XGBoost) et enregistre tous les hyperparamètres, métriques d\'entraînement et le modèle résultant sans code explicite. Cela réduit le risque d\'oublier de logger des informations critiques pour la reproductibilité.',
    difficulty: 'easy',
  },
];
